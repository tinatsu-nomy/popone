...
fn read_axis_config(doc: &FbxDocument) -> FbxAxisConfig {
    ...
    let is_identity = (axis_mat - Mat3::IDENTITY)
        .to_cols_array()
        .iter()
        .all(|&v| v.abs() < 1e-6);
        ...
        let layer_ids: Vec<i64> = scene
            ...
            .filter(|&&id| {
                scene
                    .objects
                    .get(&id)
                    .is_some_and(|o| o.class == "AnimationLayer")
            })
            .copied()
            .collect();
            ...
            let curve_node_ids: Vec<i64> = scene
                ...
                .filter(|&&id| {
                    scene
                        .objects
                        .get(&id)
                        .is_some_and(|o| o.class == "AnimationCurveNode")
                })
                .copied()
                .collect();
            ...
            for &cn_id in &curve_node_ids {
                ...
                for conn in &scene.connections {
                    if conn.child_id == cn_id && conn.conn_type == ConnectionType::OP {
                        if let Some(ref prop) = conn.property {
                            if prop.starts_with("Lcl Rotation")
                                || prop.starts_with("Lcl Translation")
                            {
                ...
                let prop_type = target_property.as_deref().or_else(|| {
                    ...
                    if name.contains("Rotation") || name.contains("R") {
                        Some("Lcl Rotation")
                    } else if name.contains("Translation") || name.contains("T") {
                ...
                let curve_ids: Vec<i64> = scene
                    ...
                    .filter(|&&id| {
                        scene
                            .objects
                            .get(&id)
                            .is_some_and(|o| o.class == "AnimationCurve")
                    })
                    .copied()
                    .collect();
                ...
                for axis in &["d|X", "d|Y", "d|Z"] {
                    for conn in &scene.connections {
                        if conn.parent_id == cn_id
                            && conn.conn_type == ConnectionType::OP
                ...
                match prop_type {
                    ...
                    Some(p) if p.starts_with("Lcl Translation") => {
                        let keyframes =
                            build_translation_keyframes(&axis_curves, &mut duration, axis_config);
                        if !keyframes.is_empty() {
                            let entry =
                                bone_channels
                                    .entry(bone_name)
...
fn collect_bone_pre_rotations(scene: &FbxScene) -> HashMap<String, Quat> {
    ...
    for obj in scene.objects.values() {
        if obj.class == "Model"
            && matches!(obj.sub_type.as_str(), "LimbNode" | "Root" | "Null" | "")
        {
    ...
    let non_identity_count = pre_rots
        .values()
...
fn read_animation_curve(node: &super::parser::FbxNode) -> (Vec<f32>, Vec<f32>) {
    let times: Vec<f32> = node
        ...
        .map(|arr| {
            arr.iter()
                .map(|&t| (t as f64 / FBX_TIME_UNIT) as f32)
                .collect()
        })
        .unwrap_or_default();
...
fn collect_bone_rests(
    scene: &FbxScene,
    axis_config: &FbxAxisConfig,
) -> (HashMap<String, VrmaBoneRest>, HashMap<String, Vec3>) {
    use super::bone::extract_transform;
    use glam::Mat4;
    ...
    fn compute_global(id: i64, scene: &FbxScene, globals: &mut HashMap<i64, Mat4>) -> Mat4 {
        ...
        let obj = match scene.objects.get(&id) {
            Some(o) => o,
            None => {
                globals.insert(id, Mat4::IDENTITY);
