...
pub(crate) fn extract_polygon_indices(geom: &FbxNode) -> Option<&[i32]> {
    let node = geom.child("PolygonVertexIndex")?;
    node.properties.first()?.as_i32_array()
}
...
pub(crate) fn extract_normal_indices(geom: &FbxNode) -> &[i32] {
    ...
    prop.as_i32_array().unwrap_or(&[])
}

pub(crate) fn extract_material_indices(geom: &FbxNode) -> &[i32] {
...
pub(crate) fn extract_uvs(geom: &FbxNode) -> (Option<VertexData<'_>>, &[i32], UvMapping) {
    let Some(layer) = geom.child("LayerElementUV") else {
        return (None, &[], UvMapping::ByPolygonVertex);
    };
    ...
    let uv_idx = match mapping {
        UvMapping::ByPolygonVertex => {
            if !uv_indices.is_empty() {
                uv_indices.get(poly_vert_idx).copied().unwrap_or(0) as usize
            } else {
                poly_vert_idx
            }
        }
        UvMapping::ByControlPoint => {
            if !uv_indices.is_empty() {
                uv_indices.get(control_point_idx).copied().unwrap_or(0) as usize
            } else {
                control_point_idx
            }
        }
...
pub(crate) fn extract_material_props(mat_node: &FbxNode) -> MaterialProps {
    ...
    for p in &p70.children {
        ...
        match name {
            ...
            "ReflectionColor" => {
                props.reflection_color[0] = p
                    .properties
                    .get(4)
                    .and_then(|v| v.as_f64_value())
                    .unwrap_or(0.0) as f32;
                props.reflection_color[1] = p
                    .properties
                    .get(5)
                    .and_then(|v| v.as_f64_value())
                    .unwrap_or(0.0) as f32;
                props.reflection_color[2] = p
                    .properties
                    .get(6)
                    .and_then(|v| v.as_f64_value())
                    .unwrap_or(0.0) as f32;
