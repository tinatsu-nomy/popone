...
fn extract_bones(pmx: &PmxModel) -> Vec<IrBone> {
    ...
    for i in 0..bones.len() {
        if let Some(parent) = bones[i].parent {
            if parent < bones.len() {
                // 安全なインデックスアクセスのため一旦インデックスを記録
                let _ = parent; // children は別ループで構築
            }
        }
    }
    // 2パス: children 構築
        ...
        for fi in face_offset..face_offset + face_count {
            ...
            for &global_idx in &reordered {
                let local_idx = if let Some(&existing) = vertex_map.get(&global_idx) {
                    ...
                    let weights = match &v.weight {
                        PmxWeightType::Bdef1 { bone } => {
                            vec![(*bone as usize, 1.0)]
                        }
                        PmxWeightType::Bdef2 { bone1, bone2, weight1 } => {
                            let mut w = vec![(*bone1 as usize, *weight1)];
                            if *weight1 < 1.0 {
                                w.push((*bone2 as usize, 1.0 - weight1));
                            }
                            w
                        }
...
fn distribute_vertex_morphs(pmx: &PmxModel, meshes: &mut [IrMesh]) {
    ...
    for morph in &pmx.morphs {
        if let PmxMorphOffsets::Vertex(offsets) = &morph.offsets {
            // 各メッシュに対するモーフターゲットを構築
            let mut mesh_offsets: Vec<Vec<Option<Vec3>>> = meshes
                .iter()
                .map(|m| vec![None; m.vertices.len()])
                .collect();
            ...
            for off in offsets {
                ...
                if let Some(targets) = global_to_local.get(&off.vertex_index) {
                    for &(mesh_idx, local_idx) in targets {
                        mesh_offsets[mesh_idx][local_idx as usize] = Some(gltf_offset);
                    }
                }
            }

            for (mesh_idx, offsets) in mesh_offsets.into_iter().enumerate() {
                // このメッシュに影響がある場合のみ追加
                if offsets.iter().any(|o| o.is_some()) {
...
/// PMX剛体回転の調整: ボーン方向が下向き（Y<0）の場合、X回転を反転
fn adjust_pmx_rigid_rotation(pmx: &PmxModel, bone_index: Option<usize>, rot: Vec3) -> Vec3 {
    let Some(bi) = bone_index else { return rot; };
    if bi >= pmx.bones.len() { return rot; }
    let bone = &pmx.bones[bi];

    let dir = match &bone.tail {
        BoneTail::BoneIndex(child_idx) => {
            let ci = *child_idx as usize;
            if ci < pmx.bones.len() {
                pmx.bones[ci].position - bone.position
            } else {
                return rot;
            }
        }
        BoneTail::Offset(off) => *off,
    };
...
fn extract_physics(pmx: &PmxModel) -> IrPhysics {
    let rigid_bodies = pmx
        ...
        .map(|r| {
            ...
            IrRigidBody {
                ...
                rotation: adjust_pmx_rigid_rotation(pmx, bone_index, r.rotation),
                mass: r.mass,
    ...
    let joints = pmx
        ...
        .map(|j| IrJoint {
            name: j.name.clone(),
            rigid_a: if j.rigid_a >= 0 { j.rigid_a as usize } else { 0 },
            rigid_b: if j.rigid_b >= 0 { j.rigid_b as usize } else { 0 },
...
mod tests {
    ...
    fn test_pmx_to_ir_seed_san() {
        ...
        for tex in &ir.textures {
            assert!(!tex.data.is_empty(), "テクスチャ '{}' のデータが空", tex.filename);
        }
    }
}
