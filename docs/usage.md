...
- **Appearance Presets (v0.5.17)** — The appearance settings window, opened from the "Appearance" button at the right end of the top bar, switches between System / Light / Dark / Custom. The default is the Blender / Substance Painter style dark theme (unified color scheme for panels, buttons, and tooltips). "System" follows the OS light/dark setting live at runtime. The color pickers for six colors (background, buttons, text, border, accent, active) are always visible and reflect the current preset; editing any color automatically switches the preset to "Custom" based on the colors shown at that moment (the "Reset colors" button restores the defaults). Side panel defaults to 280px (draggable within 280–600px when the panel-resize toggle in the Display tab is on) with flat tab bar. Displays a rounded-corner splash image centered in the viewport when no model is loaded
...
- **Normal Tools** — Normal smoothing ✨, custom normal clear 🗑 (compatible with normal maps: smoothing TBN base normals improves polygon edge visibility), normal map ON/OFF 🗺, normal direction visualization. v0.5.3 adds bulk `Smooth normals [on][off]` / `Clear custom normals [on][off]` button rows above the material list.
...
- **Editable Model Name (PMX output filename + title bar)** — A "Model name" text input in the top bar and another in the right-side "PMX Conversion" panel share the same value, and changes are reflected immediately in the window title, the PMX output filename, and the default UV map export filename. The initial value is determined by the load source: single file → file name; single Prefab → Prefab name; archive (zip / 7z / rar / unitypackage) → archive file name. Appended models preserve the name decided at the first load. The edited name is also retained across reloads (A/T stance toggling, etc.)
...
- **Texture slot assignment (refactored in v0.5.2)** — Each parameter section now shows the related texture thumbnail (32px square) as a button at its top:
  - **Basic**: BaseColor
  - **Shade**: Shade / ShadingShift
  - **Outline**: OutlineWidth
  - **Rim**: RimMultiply
  - **MatCap**: Matcap
  - **UV Animation**: UvAnimMask
  - **Emissive / Normal**: Emissive / Normal
...
- **MToon enable/disable** — "Enable MToon" checkbox to promote a non-MToon material
...
- **Reset** — Per-slot `×` button to clear individual textures, "Reset to defaults" to restore the material to its load-time state
...
- **PMX non-support badges (visually strengthened in v0.5.1)** — Parameters not representable in PMX format show a color-coded `⚠ Not in PMX` badge at the top of the relevant section with a hover tooltip explaining that MME (.fx) output and viewer preview do honor them

### Per-Vertex UV Editor (v0.5.5 – v0.5.6)

The "UV Edit" button in the material editor header opens a dedicated window for **per-vertex** UV editing on the active material. It complements the per-slot UV transform (v0.5.4) for finer adjustment and supports editing of PMX UV morphs.
...
- **Vertex pick & drag** — Click within 12 px of a vertex to select (highlighted yellow); dragging translates. Edits are applied directly to the loaded model and are preserved in the PMX output when converting from VRM / FBX etc.
...
- **PMX UV morph editing (v0.5.5 Phase 3 A-2 / export write-back added in v0.5.6)** — The "Target" ComboBox at the top switches between "Base UV" and any PMX UV morph. Selecting a morph lets you edit its per-vertex offsets:
  ...
  - **Side-panel slider lock (v0.5.6)** — While a UV morph is being edited, the matching row in the "Expression Morphs" side panel disables its slider, `0` / `1` buttons, and DragValue, and shows an "(editing UV)" marker. The "Reset all" button also skips it
  - **UV morph write-back on PMX export (v0.5.6)** — The PMX export path can now write edited UV morphs back as `PmxMorphOffsets::Uv` (v0.5.5 stubbed them out as empty group morphs). Note that PMX / PMD models are view-only — the GUI cannot re-export them to PMX (see "Notes / Limitations") — so to keep UV morph edits made on a PMX-loaded model, use "Save history" (`popone_history.json`)
- **History persistence** — "Save history" writes per-vertex UV deltas to `popone_history.json`; "Recall history" restores them. Stored per model path

### MME (ray-mmd) Output (v0.5.0)

Generate ray-mmd 2.0 material `.fx` files alongside PMX conversion.

1. In the Export tab, check "Also output MME materials (.fx)" under the PMX conversion section
2. (Optional) Click "Select folder..." to set the ray-mmd root folder. Defaults to the current directory (`.\`) if not set
...
- **Manual override** — Open the material editor and expand the "MME export (ray-mmd)" section to manually select a category via ComboBox
