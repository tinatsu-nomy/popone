...
- **Session Settings Persistence** — Window size/position, last-opened directories, and log settings saved to `popone.toml` (in `%LOCALAPPDATA%\popone` on Windows) and restored on next launch. Log level (`[log] level`) and log file retention count (`[log] keep`) are configurable. Multi-display support
