...
- **セッション設定の永続化** — ウィンドウのサイズ・位置、最後に開いたディレクトリ、ログ設定、ログビュアー設定、テーマカラーを `popone.toml`（Windows では `%LOCALAPPDATA%\popone`）に保存し次回起動時に復元。ログレベル（`[log] level`）を設定可能（v0.4.0 でログ自動ローテーションが廃止されたため `[log] keep` は無効）。ログビュアーの表示状態・位置・サイズ・レベルフィルタは `[log_viewer]` セクションで永続化（v0.5.9 より、別ファイル `log_viewer.toml` 内の `[window]` サブセクションは `popone.toml` 本体と同一の形式に統一されている）。右側パネル幅は `[window] right_panel_width` で永続化される（v0.5.9）。テーマカラー（`[theme]` セクション: `panel_bg` / `border` / `accent` / `text` / `widget_bg` / `active`）を 6 桁 hex 値でカスタマイズ可能。マルチディスプレイ対応
...
- **モデル名編集（PMX 出力ファイル名 + タイトルバー）** — トップバーの「モデル名」入力欄と右側パネル「PMX 変換」の「モデル名」入力欄が同じ値を共有し、ウィンドウタイトル・PMX 出力ファイル名・UV マップ出力のデフォルトファイル名に即時反映される。初期値はロード元に応じて自動決定: 単独ファイル → ファイル名、単独 Prefab → Prefab 名、アーカイブ (zip/7z/unitypackage) → アーカイブファイル名。モデル追加 (append) 時は最初にロードしたモデルの名前が維持される。リロード（A/T スタンス変換切替等）後も編集内容は保持される
...
- VRM 0.0 / 1.0 / FBX / UnityPackage / ZIP / 7z / RAR を自動判定
...
- **頂点ピック & ドラッグ** — 頂点から 12 px 以内をクリックで選択（黄色）、ドラッグで平行移動。編集結果は `IrMesh.vertices_mut()[*].uv` に直接反映され、PMX 再出力でも保持
...
- **PMX UV モーフ編集（v0.5.5 Phase 3 A-2 / v0.5.6 でラウンドトリップ対応）** — ウィンドウ上部の「編集対象」ComboBox で「ベース UV」か PMX UV モーフを選択。モーフを選ぶと、そのモーフの頂点別オフセットを編集できる:
  ...
  - **PMX 保存との往復（v0.5.6）** — 編集した UV モーフは PMX 再出力時に `PmxMorphOffsets::Uv` として書き戻されるため、「PMX 読込 → UV モーフ編集 → PMX 保存 → 再読込」で編集結果が保たれる（v0.5.5 までは空のグループモーフに潰されていた）
- **履歴永続化** — 「履歴を保存」で頂点別 UV 差分を `popone_history.json` に書き出し、「履歴呼出」で復元。モデルパスごとに管理される
