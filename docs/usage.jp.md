...
- **セッション設定の永続化** — ウィンドウのサイズ・位置、最後に開いたディレクトリ、ログ設定を `popone.toml`（Windows では `%LOCALAPPDATA%\popone`）に保存し次回起動時に復元。ログレベル（`[log] level`）とログファイル保持数（`[log] keep`）を設定可能。マルチディスプレイ対応
