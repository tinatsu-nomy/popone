...
- **外観プリセット（v0.5.17）** — 表示タブの「外観」セレクタで「システム / ライト / ダーク / 任意」を切替可能。既定は Blender / Substance Painter 風のダークテーマ（パネル・ボタン・ツールチップを統一配色）。「システム」は OS のライト / ダーク設定に実行中も自動追従し、「任意」はダーク配色をベースに 6 色（背景・ボタン・文字・枠線・アクセント・アクティブ）をカラーピッカーでカスタマイズできる。サイドパネル 280px 固定幅、フラットタブバー。モデル未ロード時はビューポート中央にスプラッシュ画像を角丸表示
