...
- **UVマップ出力** — 材質レイヤー分けの PSD として出力（1024〜8192 解像度）。UV 境界をまたぐ三角形のラップ描画対応。複数モデルマージ時はモデル別にレイヤーグループフォルダに格納。保存ダイアログのデフォルトディレクトリは読み込んだモデルのソースディレクトリ（アーカイブ経由の場合はアーカイブ本体の置き場所）、デフォルトファイル名はトップバーの「モデル名」欄から自動生成される `{モデル名}.psd`
