...
- **外観プリセット（v0.5.17）** — トップバー右端の「外観」ボタンで開く外観設定ウィンドウから「システム / ライト / ダーク / 任意」を切替可能。既定は Blender / Substance Painter 風のダークテーマ（パネル・ボタン・ツールチップを統一配色）。「システム」は OS のライト / ダーク設定に実行中も自動追従し、「任意」は切替時点の配色を初期値として 6 色（背景・ボタン・文字・枠線・アクセント・アクティブ）をカラーピッカーでカスタマイズできる（「色を初期値に戻す」でデフォルト値に復帰）。サイドパネル 280px 固定幅、フラットタブバー。モデル未ロード時はビューポート中央にスプラッシュ画像を角丸表示
