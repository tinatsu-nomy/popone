...
- **Appearance presets** — The View tab gained an "Appearance" selector with four presets: System / Light / Dark / Custom. "System" follows the OS light/dark setting live at runtime. "Dark" is the previous look (still the default). "Light" is a new light color scheme. "Custom" overlays the six `[theme]` colors from `popone.toml` (background, buttons, text, border, accent, active) on the dark base — those six colors used to be a hand-edit-only hidden setting and are now editable via GUI color pickers (with a "Reset colors" button). The selected preset and custom colors are persisted to `popone.toml`. Existing configs with hand-edited `[theme]` colors are automatically treated as "Custom", so their look does not change. The log viewer, text viewer, and panel text colors follow the theme as well. The 3D viewport background remains independently controlled by the "Background" slider.
