...
- **Appearance presets** — The appearance settings window, opened from the "Appearance" button at the right end of the top bar, switches the app colors between four presets: System / Light / Dark / Custom. "System" follows the OS light/dark setting live at runtime. "Dark" is the previous look (still the default). "Light" is a new light color scheme. "Custom" makes the six `[theme]` colors from `popone.toml` (background, buttons, text, border, accent, active) editable via GUI color pickers — those six colors used to be a hand-edit-only hidden setting. Switching to Custom starts editing from the look in effect at that moment (e.g. the light colors when Light is showing); the "Reset colors" button restores the default (dark) values. Light/dark behavior is inferred from the background luminance, so bright custom palettes keep readable text. The selected preset and custom colors are persisted to `popone.toml`. Existing configs with hand-edited `[theme]` colors are automatically treated as "Custom", so their look does not change. The log viewer, text viewer, and panel text colors follow the theme as well. The 3D viewport background remains independently controlled by the "Background" slider on the View tab.
