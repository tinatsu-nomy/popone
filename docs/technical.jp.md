...
- [技術詳細](#%E6%8A%80%E8%A1%93%E8%A9%B3%E7%B4%B0)
  ...
  - [アーカイブ直接ロード](#%E3%82%A2%E3%83%BC%E3%82%AB%E3%82%A4%E3%83%96%E7%9B%B4%E6%8E%A5%E3%83%AD%E3%83%BC%E3%83%89)
    ...
    - [パスワード付きアーカイブ（v0.5.15）](#%E3%83%91%E3%82%B9%E3%83%AF%E3%83%BC%E3%83%89%E4%BB%98%E3%81%8D%E3%82%A2%E3%83%BC%E3%82%AB%E3%82%A4%E3%83%96v0515)
    - [多段アーカイブ（v0.5.15）](#%E5%A4%9A%E6%AE%B5%E3%82%A2%E3%83%BC%E3%82%AB%E3%82%A4%E3%83%96v0515)
    - [アーカイブ内テキストビューワ（v0.5.16）](#%E3%82%A2%E3%83%BC%E3%82%AB%E3%82%A4%E3%83%96%E5%86%85%E3%83%86%E3%82%AD%E3%82%B9%E3%83%88%E3%83%93%E3%83%A5%E3%83%BC%E3%83%AFv0516)
    - [CLI](#cli)
  ...
  - [ビューア表示スタイル](#%E3%83%93%E3%83%A5%E3%83%BC%E3%82%A2%E8%A1%A8%E7%A4%BA%E3%82%B9%E3%82%BF%E3%82%A4%E3%83%AB)
    - [外観テーマ（v0.5.17: システム / ライト / ダーク / 任意）](#%E5%A4%96%E8%A6%B3%E3%83%86%E3%83%BC%E3%83%9Ev0517-%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0--%E3%83%A9%E3%82%A4%E3%83%88--%E3%83%80%E3%83%BC%E3%82%AF--%E4%BB%BB%E6%84%8F)
...
- **linear 色空間リサイズ** — sRGB u8 を linear f32 に変換（LUT で高速化）してから `image::imageops::resize`（Triangle フィルタ）で縮小し、linear → sRGB u8 に戻す（`viewer/texture.rs` の `resize_srgb` / ミップチェーン生成ループ、`color.rs` の `rgba8_to_linear_f32` / `linear_f32_to_rgba8`）。sRGB 空間のまま縮小すると暗部が沈む誤差が出るため、色正確性を優先
...
GUID ベースのインデックス構造で、GUID から pathname・data・meta への O(1) 参照を実現する。テクスチャ解決を O(1) にするための構築済み Prefab キャッシュも保持する。
...
pub struct UnityPackageIndex {
 ...
 /// FBX GUID → .prefab エントリ index 群
 pub prefab_by_fbx_guid: HashMap<String, Vec<usize>>,
 /// Prefab パース結果キャッシュ: エントリ index → ParsedPrefabCache
 pub prefab_cache: HashMap<usize, ParsedPrefabCache>,
 /// Variant 解決キャッシュ: ソース GUID → 解決済み FBX GUID 群
 pub variant_cache: HashMap<String, Vec<String>>,
}
```

`build_unity_package_index()` で tar.gz を一度だけ展開し、以降は `by_guid` / `by_path` で参照。
ビューア直接読み込みとアーカイブ（ZIP / 7z / RAR）経由の両方で構築される。

インデックス構築後、`build_prefab_fbx_map()` が Prefab キャッシュを構築する:
1. **Phase 1（並列）**: 全 `.prefab` エントリを `rayon::par_iter` でパース（`detect_prefab_format` + `parse_prefab_new` + `parse_prefab_old`。Mixed 形式は常に両パーサーを実行）
2. **Phase 2a**: パース結果を `prefab_cache` に格納
3. **Phase 2b（逐次）**: Variant 解決（`resolve_variant_multi`）で各 Prefab の FBX 参照を `prefab_by_fbx_guid` にマップ。結果は `variant_cache` にキャッシュ

`resolve_prefab_textures()` は全エントリ走査の代わりに `prefab_by_fbx_guid.get(fbx_guid)` で O(1) 参照し、再パースの代わりに `prefab_cache` から読む。

`TextureData::Encoded` は `Arc<[u8]>` を使い、`PackageTexture.data` のテクスチャデータをコピーなしで共有する。
...
| Archive (ZIP/7z/RAR) | `archive.zip` → `entry.vrm` → テクスチャ |
...
ZIP / 7z / RAR アーカイブ内のモデルファイルを検出・展開する統一 API。
...
7z は `sevenz-rust2` のストリーミング API の制約上、`list_models` 時点で対象拡張子のファイルを全展開してメモリに保持する（`MAX_TOTAL_BYTES = 2GB` 上限）。ソリッドブロック内のスキップ対象エントリも `io::sink` へ読み捨てて CRC 整合を保つ。展開済みエントリは `ArchiveContents` 内に保持され、`extract_model_bundle` で再展開なく利用される。RAR も同様にフィルタ付き全展開方式（`archive/rar.rs` の `extract_filtered`。`unrar` crate がファイルパス入力のみのため一時ファイル経由）。
...
ZIP / 7z / RAR 内の `.unitypackage` を検出し、二重展開で内部の VRM / FBX を読み込む。
...
### パスワード付きアーカイブ（v0.5.15）

暗号化された ZIP（ZipCrypto / AES）/ 7z / RAR に対応。GUI 専用機能で、パスワード必須と判定されるとビューアが入力ダイアログを表示する。入力されたパスワードはそのロードにのみ使用し、保存しない（リロード時は再入力）。CLI で暗号化アーカイブを指定した場合は GUI の利用を案内するエラーを返す。RAR はヘッダ暗号化・コンテンツ暗号化の両方に対応（RAR4 はパスワード検証値を持たないため、誤パスワードは CRC エラー等として表面化することがある）。

### 多段アーカイブ（v0.5.15）

アーカイブ内アーカイブ（1 段）の展開に対応。内側アーカイブがパスワード付きの場合も、外側アーカイブのテキストファイル（パスワードが記載されていることの多い readme 等）はパスワードダイアログと同時に一覧表示される。

### アーカイブ内テキストビューワ（v0.5.16）

`viewer/text_viewer.rs`。ZIP / 7z / RAR に同梱されたテキストファイル（`.txt` / `.md`）を一覧化し、クリックで OS レベルの別ウィンドウ（`show_viewport_deferred`）に表示する。文字コードは UTF-8（BOM あり / なし）・UTF-16（BOM）・Shift_JIS を自動判定し、改行コードを正規化。1 ファイル 4 MB 超は対象外、表示は先頭 100 万文字まで。一覧はアーカイブ読み込みごとに置き換え（アペンド時は追加）、通常モデルのロード後も次のアーカイブ読み込みまで保持される。
...
**進捗トースト:**
- `PendingMultiLoad.total_count` が選択モデルの総数を保持
- `PendingPkgModelLoad.batch_progress: Option<(usize, usize)>` がデキュー時点の `(current, total)` を運ぶ
- 進捗は `PendingPkgModelLoad` 自身に保持する（`multi_load` から導出しない）ため、最後の 1 件のクリーンアップ後も表示が保たれる
- 各モデルの成功時に「読み込み完了 (N/M): ファイル名」のトースト（`viewer.toast.progress.loaded`）を表示する
...
v0.5.5 の UV エディタ（Phase 3 A-2）は PMX UV モーフの取り込み・編集・実行時合成まで実装済みだったが、PMX writer 側は `IrMorphKind::Uv` を空 Group モーフとしてスタブ化しており、UV モーフは PMX 出力に反映されなかった。v0.5.6 で writer の書き戻しを完成。ただし PMX / PMD 読込モデルは GUI の PMX 変換ボタンが無効（閲覧専用、「制限事項」参照）のため、PMX 読込モデルからこの書き戻しを GUI で実行する経路は存在しない。編集結果の保存は「履歴保存」（`popone_history.json`）を使う。
...
### 外観テーマ（v0.5.17: システム / ライト / ダーク / 任意）

配色は `viewer/theme.rs` の `ThemePalette` に一元化され、トップバー右端の「外観」ボタンで開く外観設定ウィンドウからプリセット（システム / ライト / ダーク / 任意）を切替できる。egui 0.31 の `ctx.set_visuals_of(Theme::Dark / Light, …)` で両テーマの `Visuals` を登録し、`ctx.set_theme(ThemePreference)` で追従モードを設定。「システム」は winit の `ThemeChanged` 経由で OS のライト / ダーク切替に実行中も追従する。実効パレットは毎フレーム `ctx.theme()` から解決して `ViewerApp.palette` に保持し、各パネル（トップバー・サイドパネル・ステータスバー）は `egui::Frame::new().fill().stroke()` でパレット色を明示指定（egui のパネルデフォルトフレーム生成をバイパス）。

デフォルト（ダーク）は Blender / Substance Painter 風:

| 要素 | ダーク | ライト |
|------|------|------|
| パネル背景 | `#1D1D1D` | `#E2E2E2` |
| セクションヘッダ / open | `#2A2A2A` | `#D5D5D5` |
| ウィジェット背景 | `#252525` | `#DADADA` |
| ボーダー | `#333333` | `#BBBBBB` |
| アクセント（選択・ホバー） | `#4A90D9` | `#4A90D9`（共通） |
| アクティブ（押下中） | `#2A5A8A` | `#A9C7E8` |
| テキスト（強調） | `#FFFFFF`（`override_text_color`） | `#101010` |

ライトの中立灰はダークの反転（`0x1D → 0xE2` 等）。テーマ対応テキスト色は `theme::gray_text(dark, level)`（ライトでは `255 - level` に反転）/ `strong_text(dark)` / `accent_text(dark, c)`（ライトでは 0.55 倍暗色化）で解決する。「任意」は `[theme]` の 6 色を適用するモードで、カラーピッカーは常時表示され、色を変更するとその時点の配色をベースに自動で「任意」へ fork する。明暗は panel_bg の輝度（Rec.601、閾値 128）で自動判定し、`open_bg` / `extreme_bg` は panel_bg からのオフセット（ダーク: +0x0D / -0x08、ライト: -0x0D / +0x12）で導出する（このオフセットは組込両パレットを正確に再現する）。3D ビューポートのクリアカラーは egui テーマと独立（表示タブ「背景」スライダーの `bg_brightness`）。
...
- サイドパネル幅は既定でリサイズ無効（`resizable(false)` + `width_range` を現在幅に固定）。表示タブの「パネルリサイズ」トグルを ON にすると `width_range(280.0..=600.0)` でドラッグ変更でき、幅は `[display] panel_width` に永続化される。OFF に戻すとその時点の幅で固定される（初期値 280px）
...
データ層（`ir.comment`）は英語ラベルを保持し、PMX コメントフィールドに英語のまま出力される。UI 表示時にのみ `meta_section_ja()` / `meta_label_ja()`（歴史的な名前だが内部は `t!()` で現在の UI 言語 ja/en/zh に変換）でローカライズし、`meta_label_tooltip()` / `format_meta_value()` でツールチップとバッジを付与する。
...
- **ログ設定**: `[log]` セクションで `level`（error/warn/info/debug、デフォルト: debug）を設定可能。設定はロガー初期化前に読み込まれるため、最初のログメッセージから反映。不正な `level` 値は `debug` にフォールバック。旧 `keep`（ログファイル保持数）は v0.4.0 のログ自動ローテーション廃止に伴い削除済みで、記述しても無視される
- **外観テーマ**: `[theme]` セクション。`mode`（`system` / `light` / `dark` / `custom`、v0.5.17）が外観プリセットを指定する。`mode` 未指定の旧設定は「色指定があれば `custom`、なければ `dark`」と解釈される（`ThemeConfig::effective_mode()`、後方互換）。カスタム 6 色はオプション指定: `panel_bg`（デフォルト: `1D1D1D`）、`border`（`333333`）、`accent`（`4A90D9`）、`text`（`D0D0D0`）、`widget_bg`（`252525`）、`active`（`2A5A8A`）。`"RRGGBB"` または `"#RRGGBB"` 形式を受容し、`ThemeConfig::parse_hex()` が `#` プレフィックスを除去し 6 文字長を検証して `(u8, u8, u8)` を返す。`mode = "custom"` のときのみ適用され、`theme::apply()` が `ThemePalette` 経由で egui `Visuals` に反映する（詳細は「外観テーマ」セクション参照）。プリセットとカスタム色は GUI の外観設定ウィンドウからも編集され、終了時に保存される
...
├── color.rs sRGB ↔ linear 色空間変換ヘルパー（LUT 高速化、ミップ生成・リサイズで使用）
├── psd.rs PSD デコーダー（PSD → RGBA 展開、CLI の PNG 変換とビューア表示で共用。PSD / PSB writer は convert/uvmap.rs）
├── unitypackage.rs .unitypackage (tar.gz) アセット展開 + Prefab テクスチャマッピング（GUID 解決・Variant 再帰・複数形式対応）
├── archive/
│ ├── mod.rs ZIP / 7z / RAR 統一 API（list_models, extract_model_bundle、多段アーカイブ・パスワード対応）
│ ├── zip_extract.rs ZIP 展開（2パス: メタデータ一覧→選択展開、ZipCrypto / AES 暗号化対応）
│ ├── sevenz.rs 7z 展開（フィルタ付き全展開、チャンク読み込み上限付き、ソリッドブロック drain、暗号化対応）
│ └── rar.rs RAR 展開（unrar crate、公式 UnRAR ライブラリ、暗号化対応）
├── vrm/
...
├── directx/
│ ├── parser.rs DirectX .x テキスト形式パーサー（Shift_JIS / UTF-8。バイナリ・圧縮形式は明示エラー）
│ └── extract.rs DirectX .x → 中間表現（IrModel）抽出
├── pmx/
│ ├── types.rs PMX データ型定義
...
│ ├── tangent.rs MikkTSpace 接線生成（mikktspace crate、縮退メッシュ事前検出）
...
└── viewer/ ← feature = "viewer" 時のみコンパイル
 ├── app/ eframe::App 状態管理（9分割）
 │ ├── mod.rs ViewerApp 構造体定義・初期化・eframe::App impl
 │ ├── file_io.rs ファイル読み込み・D&D・リロード・バックグラウンドロード
 │ ├── texture_mgmt.rs テクスチャ割り当て・プレビュー
 │ ├── material_edit.rs 材質編集ドロワー状態（パラメータオーバーライド・ダーティ管理）
 │ ├── material_presets.rs 材質プリセット（MToon 1.0 / lilToon / PMX 互換）
 │ ├── uv_edit.rs 頂点単位 UV 編集ウィンドウ状態（UV モーフ編集・undo/redo）
 │ ├── persistence.rs 設定永続化（popone.toml / popone_history.json、Named Mutex 排他）
 ...
 ├── bloom.rs Bloom ポストエフェクト（Dual Kawase、emissive MRT 分離）
 ├── mesh.rs IrModel → GPU 頂点バッファ変換
 ├── texture.rs テクスチャ GPU アップロード（MIME ヒント対応、linear 空間ミップ生成）
 ├── camera.rs オービットカメラ
 ├── grid.rs グリッド床
 ├── theme.rs 外観テーマ（システム / ライト / ダーク / 任意、ThemePalette → egui Visuals）
 ├── ui.rs 情報パネル・モーフスライダ・変換ボタン・PMX/PMD グレーアウト
 ├── log_viewer.rs ログビュアー別ウィンドウ（show_viewport_deferred、レベルフィルタ・自動追尾）
 ├── text_viewer.rs アーカイブ内テキストビューワ（.txt / .md 一覧・別ウィンドウ表示・文字コード自動判定）
 ├── watchdog.rs メインスレッド応答監視（ハートビート、フリーズ検出ログ）
...
# ビューア込みのテスト（viewer feature はデフォルト無効のため別途必要。CI は両方実行）
