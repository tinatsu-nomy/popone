...
- **HDR 自動検出**: `emissive_factor` の任意成分が 1.0 を超える材質はデフォルト OFF で初期化。トーンマッピング非搭載のビューアで白飛びを自動回避
...
| `[B]` | `emissive_per_mat` | グループ内の emissive 付き材質のエミッシブを一括トグル。OFF にすると `emissive_factor` がゼロ化され、`lit += emissive` と MRT Bloom 出力の両方が無効。HDR emissive（成分 > 1.0）はデフォルト OFF |
...
exe と同じディレクトリに配置。ウィンドウの位置（`outer_rect` 座標）・サイズ（`inner_rect` 座標）・最後に開いたディレクトリを保存する。
