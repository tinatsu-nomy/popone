...
- [技術詳細](#%E6%8A%80%E8%A1%93%E8%A9%B3%E7%B4%B0)
  ...
  - [非同期モデル読み込み (v0.2.27, v0.2.28 でキャンセル/世代管理を追加)](#%E9%9D%9E%E5%90%8C%E6%9C%9F%E3%83%A2%E3%83%87%E3%83%AB%E8%AA%AD%E3%81%BF%E8%BE%BC%E3%81%BF-v0227-v0228-%E3%81%A7%E3%82%AD%E3%83%A3%E3%83%B3%E3%82%BB%E3%83%AB%E4%B8%96%E4%BB%A3%E7%AE%A1%E7%90%86%E3%82%92%E8%BF%BD%E5%8A%A0)
    ...
    - [`cpu_parse_model` フリー関数 (file_io.rs)](#cpu_parse_model-%E3%83%95%E3%83%AA%E3%83%BC%E9%96%A2%E6%95%B0-file_iors)
...
### `cpu_parse_model` フリー関数 (file_io.rs)

`&self` を取らない純粋関数で、BG スレッドから安全に呼び出せる。各フォーマット（VRM / FBX / PMX / PMD / OBJ / STL / DirectX .x）のパースロジックを統一的に提供し、`(IrModel, ReloadableSource)` を返す。`preloaded: Option<PreloadedData>` をパラメータとして受け取り、D&D 時の先読みデータを利用する。v0.2.28 からは `cancel: &Arc<AtomicBool>` 引数を追加し、関数冒頭でキャンセルフラグをチェックしてセットされていれば `anyhow::bail!("bg load cancelled")` で即座に打ち切る。パーサ本体（`vrm::extract` / `fbx::extract` など）には侵入せず、ディスパッチ境界でのみチェックすることで実装の複雑さを抑えている。
...
- **GPU アクセス制限**: `cpu_parse_model` は `wgpu::Device` / `Queue` を一切参照しないフリー関数。GPU 操作はメインスレッドの `finish_load` でのみ実行
...
- **二重ロードのキャンセル（v0.2.28）**: `route_load_dispatch` は「拒否」ではなく「キャンセルして受け入れる」方式に変更。先行 `BgLoadHandle` の `cancel: Arc<AtomicBool>` を `true` にセットしてから新規 `spawn_bg_load` を実行する。先行スレッドは次回のキャンセルチェック地点（現在は `cpu_parse_model` 冒頭）で `"bg load cancelled"` を返し、受信側はそれをキャンセル由来エラーとして `log::info!` のみ記録し UI には出さない
...
- **`vrm::extract::extract_textures`**: gltf の生ピクセル（RGB → RGBA 変換込み）をそのまま `IrTexture.data` に格納。`mime_type = "image/x-raw-rgba8"`、`raw_dims = Some((width, height))` で識別
- **`IrTexture::is_raw_rgba()`**: mime_type と raw_dims の両方を確認する判定メソッド
- **`upload_textures_from_ir`**: `tex.is_raw_rgba()` が true なら PNG デコードをスキップし、`tex.data` を直接 RGBA として GPU アップロード
- **`write_all_textures_from_ir`（PMX エクスポート）**: `is_raw_rgba()` が true なら `image::RgbaImage::save` で PNG エンコードして出力
...
バックグラウンドロード経由:
  ...
  → std::thread::spawn で cpu_parse_model(path, format, ..., preloaded) を実行
