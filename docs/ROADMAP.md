...
The MToon shader side already handles per-draw material color and UV transform parameters. What is missing is the Expression playback pipeline wiring: when an Expression weight changes, the viewer must iterate active binds and write interpolated values into the corresponding `MaterialUniform` entries. PMX export of these binds is out of scope for v0.3.0 (MMD has no equivalent).
...
| ID | Feature | Scope |
|----|---------|-------|
| F-2 | Simple material editor with presets | GUI sliders for shader parameters, PMX write-back, use-case presets |
