...
- MMD モード ON: `mmd_ambient_scale = 154.0 / 255.0`（固定値）
- MMD モード OFF: `mmd_ambient_scale = ambient_intensity`（UI スライダー値）

標準シェーダーは `camera.ambient` / `camera.ambient_ground`（半球 ambient）と `camera.light_color` を使用し、MMD シェーダーのみ `camera.mmd_ambient_scale` を参照する。MMD モード ON 時は `light_color` / `ambient_sky_color` / `ambient_ground_color` の変更は反映されない。
...
| specular | `Vec3::ZERO` | `_HighColor`（抽出時に設定済み） | 変更なし |
| specular_power | `0.0` | `_HighColor_Power * 10.0` | 変更なし |
