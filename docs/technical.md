...
- `parse_material_textures()` — `m_TexEnvs` から メインテクスチャ GUID を取得。スロット優先順: `_MainTex` > `_BaseMap` > `_BaseColorMap`（lilToon 等で `_BaseColorMap` が `_MainTex` と異なるテクスチャを参照する場合の対策）
