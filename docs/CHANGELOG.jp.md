...
- **外観プリセット** — トップバー右端の「外観」ボタンで開く外観設定ウィンドウから、アプリの配色を「システム / ライト / ダーク / 任意」から選べるようになった。「システム」は OS のライト / ダーク設定に実行中も自動追従する。「ダーク」は従来どおりの配色（既定値）。「ライト」は新設のライト配色。「任意」は `popone.toml` `[theme]` の 6 色（背景・ボタン・文字・枠線・アクセント・アクティブ）を GUI のカラーピッカーで編集できるモードで（従来は手編集専用の隠し設定）、切替時点の配色（例: ライト表示中ならライトの色）を初期値として編集を始められる。「色を初期値に戻す」ボタンでデフォルト（ダーク）値に戻せる。明暗は背景色の輝度から自動判定されるため、明るい任意配色でも文字色が破綻しない。選択したプリセットとカスタム色は `popone.toml` に保存される。既存の `popone.toml` で `[theme]` の色を手編集済みの場合は自動的に「任意」として扱われ、見た目は変わらない。ログビューワ・テキストビューワ・各パネルの文字色もテーマに追従する。3D ビューポートの背景色は従来どおり表示タブの「背景」スライダーによる独立制御のまま。
