...
- **CI ワークフロー (`popone/.github/workflows/ci.yml`) の Rust 取得を `dtolnay/rust-toolchain@stable` から `@master` に変更** — `@stable` は Action 側で stable を強制するため `rust-toolchain.toml` が無視されるが、`@master` は `toolchain` input 省略時に `rust-toolchain.toml` を読み取る。これによりローカルと CI の Rust バージョンが 1 箇所（`rust-toolchain.toml`）で一元管理される。`with: components: rustfmt, clippy` の指定も `rust-toolchain.toml` 側に集約したため CI ファイルからは削除
