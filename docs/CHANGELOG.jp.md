...
- **外観プリセット** — 表示タブに「外観」セレクタを追加し、アプリの配色を「システム / ライト / ダーク / 任意」から選べるようになった。「システム」は OS のライト / ダーク設定に実行中も自動追従する。「ダーク」は従来どおりの配色（既定値）。「ライト」は新設のライト配色。「任意」はダーク配色をベースに `popone.toml` `[theme]` の 6 色（背景・ボタン・文字・枠線・アクセント・アクティブ）を上書きするモードで、従来は手編集専用の隠し設定だった 6 色を GUI のカラーピッカーで編集できるようになった（「色を初期値に戻す」ボタン付き）。選択したプリセットとカスタム色は `popone.toml` に保存される。既存の `popone.toml` で `[theme]` の色を手編集済みの場合は自動的に「任意」として扱われ、見た目は変わらない。ログビューワ・テキストビューワ・各パネルの文字色もテーマに追従する。3D ビューポートの背景色は従来どおり「背景」スライダーによる独立制御のまま。
