...
- [vrm2pmx_v0.1.14.exe](https://github.com/tinatsu-nomy/vrm2pmx/releases/download/v0.1.14/vrm2pmx_v0.1.14.exe)
...
- **MSAA（アンチエイリアス）** — 4x マルチサンプルアンチエイリアシング（UI から ON/OFF 切替可能、デフォルト ON）
- **法線平滑化** — 頂点統合（位置+UV キー）+ 法線平均化によるメッシュエッジの平滑化（UI から ON/OFF 切替可能、デフォルト OFF）。モーフデータはビルド時に GPU 空間で重複排除済み
- **法線表示** — 各頂点の法線方向をライン表示（材質表示と連動、重複除去済み、長さスライダ付き、N キーで切替）
- **カスタム法線クリア** — Maya の「法線ロック解除」/ Blender の「カスタム法線データをクリア」相当。ジオメトリから角度加重で法線を再計算。変更した法線は PMX 出力に反映
...
- **キーボードショートカット** — Ctrl+O:開く、R:カメラリセット、F:モデルにフィット、G:グリッド、B:ボーン、P:物理、W:ワイヤーフレーム、N:法線表示、L:ライトモード切替（常時表示）
- **プログレスオーバーレイ** — ファイル読み込み中・PMX 変換中に半透明オーバーレイとアニメーション付きプログレスバーを表示
- **UI 状態の自動制御** — データのないチェックボックスは自動的に選択不可（ヒューマノイドボーンなし→Aスタンス無効、剛体なし→物理関連無効、ボーンなし→ボーン表示無効、モデル未読込→PMX変換無効）
- **ログ管理** — exe と同じディレクトリの `logs/` にタイムスタンプ付きログファイルを出力（5 世代保持で自動ローテーション）。「ログ」ボタンでログフォルダを直接開ける
- **右パネル UI**
  ...
  - 材質ごとの外部テクスチャ割り当て（PNG / JPG / TGA / BMP / PSD 対応、リアルタイムプレビュー付き）
  - 複数テクスチャ一括 D&D（ファイル名と材質名の双方向マッチングで自動割り当て、Aスタンス切替後もテクスチャ設定を保持）
  - 表示設定（ライト・環境光・背景明度・グリッド・ボーン・物理・ワイヤーフレーム・ライトモード・MSAA・法線平滑化・法線表示・カスタム法線クリア）
...
- **テクスチャ** — Video ノードの埋め込みテクスチャ、または RelativeFilename による外部ファイル参照。ビューア上で材質ごとに外部テクスチャファイル（PNG / JPG / TGA / BMP / PSD）を割り当て可能。PSD は自前パーサーで統合画像のみをデコード（レイヤーセクションをスキップ）
- **零法線の自動補完** — 一部の頂点が零法線を持つ FBX モデルに対し、該当頂点のみフラット法線で補完
...
テクスチャは出力 PMX と同じディレクトリの `textures\` フォルダに PNG として出力される（PMX 内のパスは Windows バックスラッシュ区切り `textures\filename.png`）。
